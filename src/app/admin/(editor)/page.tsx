...
export default async function AdminHomePage() {
  ...
  const gaps = [
    ...
    !content.homepage.aboutLong.includes("biography") &&
      content.homepage.aboutLong.includes("will be added") &&
      "A longer biography in Issy's words",
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-3 max-w-2xl text-parchment/75">
          Update the public site after Issy answers the follow-up document. Saved
          changes appear on the website shortly. Leave unknown facts blank.
        </p>
      </div>

      <section className="rounded-xl border border-white/10 bg-navy p-6">
        <h2 className="font-display text-2xl text-parchment">Still waiting from Issy</h2>
        {gaps.length ? (
